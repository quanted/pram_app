...
def build_table(list1, list2):
    # function builds a two column table from two lists (e.g., url's and status codes)
    report = [""] * len(list1)
...
def write_report(test_name, assert_error, col1, col2):
    # method prints results of tests (two paths depending on whether test passed or failed)
    if assert_error:

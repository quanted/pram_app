...
pisces_server = os.environ['REST_SERVER_8']
...
def pisces_view(request):
    html = render_to_string('pisces/index.html', {})
...
def get_default_pisces_values(request):
    if request.method == 'GET':
        ...
        try:
            response = requests.get(
                pisces_server + "/pisces/api/Baseline?State=%s&County=%s" % (state, county)
            )
        except Exception, e:
...
def get_user_pisces_values(request):
    if request.method == 'GET':
        # Django converts GET url arguments into a dictionary available through request.GET
        ...
        response = requests.get(
            pisces_server + "/pisces/api/pisces?CapitalInvestmentScore=%s&ConsumptionScore=%s&EmploymentScore=%s&FinanceScore=%s&InnovationScore=%s&ProductionScore=%s&ReDistributionScore=%s&AirQualityScore=%s&FoodFiberAndFuelProvisioningScore=%s&GreenspaceScore=%s&WaterQualityScore=%s&WaterQuantityScore=%s&ActivismScore=%s&CommunicationScore=%s&CommunityAndFaithBasedInitiativesScore=%s&EducationScore=%s&EmergencyPreparednessScore=%s&FamilyServicesScore=%s&HealthcareScore=%s&JusticeScore=%s&LaborScore=%s&PublicWorksScore=%s&ConnectionToNatureDomainWeight=%s&CulturalFulfillmentDomainWeight=%s&EducationDomainWeight=%s&HealthDomainWeight=%s&LeisureTimeDomainWeight=%s&LivingStandardsDomainWeight=%s&SafetyAndSecurityDomainWeight=%s&SocialCohesionDomainWeight=%s"
...
# def descriptionPage(request, model='pisces'):
#
...
#     html = html + render_to_string('pisces/02uberintroblock_wmodellinks_pisces.html', {
#             'site_skin' : os.environ['SITE_SKIN'],
#             'model':model,
#             'page':'description'})
...
#
#
# def algorithmPage(request, model='pisces'):
#
...
#
#
# def referencesPage(request, model='pisces'):
#

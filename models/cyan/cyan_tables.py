...
def gett1data(hwbi_obj):
    data = { 
        "Parameter": ['LC50 or LD50', 'Threshold', 'Slope',],
        "Value": [hwbi_obj.LC50, hwbi_obj.threshold, hwbi_obj.dose_response,],
    }
    return data
...
def gett2data(hwbi_obj):
    #logger.info(vars(hwbi_obj))
    data = { 
        "Parameter": ['Z Score', '"F8"', 'Chance of Individual Effect',],
        "Value": ['%.2f' % hwbi_obj.z_score_f_out,'%.2e' % hwbi_obj.F8_f_out,'%.2f' % hwbi_obj.chance_f_out, ],
    }
    return data
...
def gett2dataqaqc(hwbi_obj):
    data = { 
        "Parameter": ['Z Score', '"F8"', 'Chance of Individual Effect',],
        "Value": ['%.2f' % hwbi_obj.z_score_f_out,'%.2e' % hwbi_obj.F8_f_out,'%.2f' % hwbi_obj.chance_f_out, ],
        "Expected Value": ['%.2f' % hwbi_obj.z_score_f_out_expected,'%.2e' % hwbi_obj.F8_f_out_expected,'%.2f' % hwbi_obj.chance_f_out_expected, ],
    }
    return data
...
def table_all(hwbi_obj):
    html_all = table_1(hwbi_obj)
    html_all = html_all + table_2(hwbi_obj)
    return html_all

def table_all_qaqc(hwbi_obj):
    html_all = table_1(hwbi_obj)
    html_all = html_all + table_2_qaqc(hwbi_obj)
    return html_all

def timestamp(hwbi_obj="", batch_jid=""):
    #ts = time.time()
    #st = datetime.datetime.fromtimestamp(ts).strftime('%A, %Y-%B-%d %H:%M:%S')
    if hwbi_obj:
        st = datetime.datetime.strptime(hwbi_obj.jid, '%Y%m%d%H%M%S%f').strftime('%A, %Y-%B-%d %H:%M:%S')
    else:
        st = datetime.datetime.strptime(batch_jid, '%Y%m%d%H%M%S%f').strftime('%A, %Y-%B-%d %H:%M:%S')
    html="""
    <div class="out_">
    <b>HWBI Version 1.0 (Beta)<br>
    """
    html = html + st
    html = html + " (EST)</b>"
    html = html + """
...
def table_2_qaqc(hwbi_obj):
        html = """
        <H4 class="out_2 collapsible" id="section2"><span></span>Model Output</H4>
            <div class="out_ container_output">
        """
        t2data = gett2dataqaqc(hwbi_obj)
...
def table_sum_output(z_score_f_out, F8_f_out, chance_f_out):
        #pre-table sum_input
        html = """
            <H4 class="out_1 collapsible" id="section3"><span></span>HWBI Outputs</H4>
                <div class="out_ container_output">
        """
        #table sum_input

...
class trexInp_chem(forms.Form):
    ...
    seed_treatment_formulation_name = forms.CharField(required=False, max_length=255, label='Seed treatment formulation name', widget=forms.Textarea (attrs={'cols': 20, 'rows': 2}))    
    seed_crop = forms.ChoiceField(required=False, label='Crop use', choices=seed_crop_CHOICES, initial='Corn')
    seed_crop_v = forms.CharField(required=False, label='Crop use verbal', widget=forms.HiddenInput)
...
class trexInp_mammal(forms.Form):
    ...
    mammalian_lc50 = forms.FloatField(required=False, label='Mammalian LC50 (mg/kg-diet)')
    mammalian_NOAEC = forms.FloatField(required=True, label='Mammalian NOAEC (mg/kg-diet)', initial=2.5)
    mammalian_NOAEL = forms.FloatField(required=True, label='Mammalian NOAEL (mg/kg-bw)', initial=0.125)

...
def read_batch_csv(csv):
    ...
    logging.info("===== rice_batchoutput.read_batch_csv")
    pd_obj = pd.read_csv(csv, index_col=0, header=None, skiprows=1, 
        skip_footer=4, engine='python')
    logging.info("===== reading rice batch csv")
    logging.info(pd_obj)
...
def riceBatchOutputPage(request):
    ...
    logging.info("==== rice_batchoutput.riceBatchOutputPage")

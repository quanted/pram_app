...
def save_batch_dic(output_html, model_object_dict, model_name, run_type, batch_jid):
    ...
    try:
        ...
        response = requests.post(url=url, data=data, headers=http_headers, timeout=6000)
    except:
        pass
...
#                MM_t=MM_t[1]            
        ...
        now = datetime.datetime(YY_t,int(MM_t),int(DD_t))
        Later = now + datetime.timedelta(days=DayRe_t)
        Later=str(Later)
